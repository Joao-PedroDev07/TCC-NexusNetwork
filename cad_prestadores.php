...
require_once("cad_prestadores_funcoes.php");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    error_log("Latitude POST: " . ($_POST['latitude'] ?? 'NÃO ENVIADO'));
    error_log("Longitude POST: " . ($_POST['longitude'] ?? 'NÃO ENVIADO'));
    error_log("Todos os POST: " . print_r($_POST, true));
    ...
    if ($is_google_signup) {
        $dados = [
            ...
            'preco_min' => filter_input(INPUT_POST, 'preco_min', FILTER_VALIDATE_FLOAT) ?: null,
            'preco_max' => filter_input(INPUT_POST, 'preco_max', FILTER_VALIDATE_FLOAT) ?: null,
            'senha' => ''
        ];
    } else {
        $dados = [
    ...
    unset($dados_validacao['descricao']);
    unset($dados_validacao['preco_min']);
    unset($dados_validacao['preco_max']);
    ...
    $resultado_foto = null;
    if (!$is_google_signup && isset($_FILES['foto']) && $_FILES['foto']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    if (empty($erros)) {
        if ($is_google_signup) {
            $dados_adicionais = [
                ...
                'preco_min' => $dados['preco_min'] ?? 0,
                'preco_max' => $dados['preco_max'] ?? 0
            ];
            
            $usuario_criado = salvarPrestadorGoogle($conn, $dados_google, $dados_adicionais);
            ...
            if (inserirPrestador($conn, $dados)) {
...
// Inicializar $dados se for cadastro Google
...
<head>
    <meta charset="utf-8">
    <title>Cadastro de Prestadores - Nexus Network</title>
    ...
    <link rel="stylesheet" href="assets/css/cad_prestador.css">
...
<body class="account-page">
                            ...
                            <div class="row align-items-center justify-content-center">
                                <div class="col-md-12 col-lg-6 login-right">
                                    <div class="login-header">
                                        ...
                                        <?php else: ?>
                                            <a href="cad_clientes.php" class="prestador">Cliente?</a>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <form method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group form-focus">
                                            <input type="text" class="form-control floating" name="profissao" 
                                                   value="<?= htmlspecialchars($dados['profissao'] ?? '') ?>" required>
                                            <label class="focus-label">Profissão</label>
                                        </div>

                                    <div class="professional-info-section">
                                        <div class="professional-header" onclick="toggleProfessionalInfo()">
                                            <div class="professional-title-wrapper">
                                                <i class="fas fa-briefcase professional-icon"></i>
                                                <h4 class="professional-title">
                                                    Informações Profissionais 
                                                    <span class="optional-badge">Opcional</span>
                                                </h4>
                                            </div>
                                            <i class="fas fa-chevron-down toggle-icon" id="professional-toggle-icon"></i>
                                        </div>
                                        
                                        <div id="professional-fields" class="professional-fields-wrapper">
                                            <p class="professional-hint">
                                                <i class="fas fa-lightbulb"></i>
                                                Complete estas informações para aumentar suas chances de contratação! 
                                                Prestadores com perfil completo recebem <strong>3x mais propostas</strong>.
                                            </p>

                                            <!-- Descrição do Negócio -->
                                            <div class="form-group form-focus">
                                                <label for="descricao">Descrição dos Seus Serviços</label>
                                                <textarea class="form-control floating" 
                                                        name="descricao" 
                                                        id="descricao"
                                                        rows="5" 
                                                        placeholder="Conte sobre sua experiência, especializações e diferenciais..."><?= htmlspecialchars($dados['descricao'] ?? '') ?></textarea>
                                                <small class="field-help">
                                                    <i class="fas fa-info-circle"></i>
                                                    Descreva o que você faz, sua experiência e o que te torna único
                                                </small>
                                            </div>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <div class="price-input-wrapper">
                                                                ...
                                                                <input type="number" 
                                                                    ...
                                                                    min="0"
                                                                    placeholder="0,00"
                                                                    value="<?= htmlspecialchars($dados['preco_min'] ?? '') ?>">
                                                            </div>
                                                        </div>
                                                    </div>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <div class="price-input-wrapper">
                                                                ...
                                                                <input type="number" 
                                                                    ...
                                                                    min="0"
                                                                    placeholder="0,00"
                                                                    value="<?= htmlspecialchars($dados['preco_max'] ?? '') ?>">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                
                                                <small class="field-help price-help">
                                                    <i class="fas fa-chart-line"></i>
                                                    Defina uma faixa de preço aproximada. Você pode ajustar depois no seu perfil!
                                                </small>
                                            </div>
                                        </div>
                                    </div>
    ...
    try {
        ...
        // USA O PROXY PHP para evitar CORS
            ...
            if (precoMin && precoMax) {
                function validarPrecos() {
                    ...
                    if (min > 0 && max > 0 && min > max) {
                        precoMax.setCustomValidity('O preço máximo deve ser maior que o mínimo');
                        precoMax.classList.add('input-invalid');
                        precoMax.classList.remove('input-valid');
                    } else {
                        precoMax.setCustomValidity('');
                        if (max > 0) {
                            precoMax.classList.add('input-valid');
                            precoMax.classList.remove('input-invalid');
                        }
                    }
                }
                
                precoMin.addEventListener('change', validarPrecos);
                precoMax.addEventListener('change', validarPrecos);
            }
        });
        ...
        function toggleProfessionalInfo() {
            ...
            const icon = document.getElementById('professional-toggle-icon');
