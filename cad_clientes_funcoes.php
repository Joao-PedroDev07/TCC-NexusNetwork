...
// Verificar se email já existe no banco (CLIENTES)
function emailJaExiste($conn, $email) {
    $sql = "SELECT cli_email FROM clientes WHERE cli_email = ?";
...
// Função para verificar CPF existente (CLIENTES)
function cpfJaExiste($conn, $cpf) {
    // Remove caracteres não numéricos antes de consultar
    $cpf_limpo = preg_replace('/\D/', '', $cpf);
    
    $sql = "SELECT cli_cpf FROM clientes WHERE cli_cpf = ?";
    $stmt = mysqli_prepare($conn, $sql);
...
// Inserir CLIENTE no banco usando prepared statement (Evita SQL Injection)
function InserirCliente($conn, $dados) {
    // Limpa o CPF antes de inserir
    $cpf_limpo = preg_replace('/\D/', '', $dados['cpf']);
    
    // Limpa o telefone antes de inserir
    $telefone_limpo = preg_replace('/\D/', '', $dados['telefone']);

    $sql = "INSERT INTO clientes (
                cli_nome, cli_datanasc, cli_email, cli_telefone, cli_genero, 
                cli_estado, cli_cidade, cli_senha, cli_cpf, cli_foto,
                cli_latitude, cli_longitude
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = mysqli_prepare($conn, $sql);
    
    // Adiciona latitude e longitude ao bind (ss no final para os dois novos campos DECIMAL)
...
function validarCamposObrigatorios($dados) {
    // Campos obrigatórios básicos (sem latitude/longitude que são opcionais)
    $campos = ['nome', 'datanasc', 'email', 'telefone', 'cpf', 'genero', 'cidade', 'estado'];
