...
if ($email != '' && $codigo != '') {
    mysqli_query($conn, "DELETE FROM reset_codigos_clientes WHERE expiracao < NOW()");

    $email_esc = mysqli_real_escape_string($conn, $email);
    $codigo_esc = mysqli_real_escape_string($conn, $codigo);

    $sql = "SELECT * FROM reset_codigos_clientes WHERE cli_email = '$email_esc' AND codigo = '$codigo_esc' AND expiracao > NOW()";
    $result = mysqli_query($conn, $sql);

    if ($result && mysqli_num_rows($result) > 0) {
        mysqli_query($conn, "DELETE FROM reset_codigos_clientes WHERE cli_email = '$email_esc'");
